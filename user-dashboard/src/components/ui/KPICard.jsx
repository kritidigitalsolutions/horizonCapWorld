...
import {
  RiArrowUpSFill, RiArrowDownSFill, RiGroupLine, RiFundsLine,
  RiExchangeDollarLine, RiMoneyDollarCircleLine, RiTimeLine, RiTrophyLine
} from 'react-icons/ri';
...
const bgMap = {
  ...
  revenue: 'bg-purple-50 text-purple-600',
  money: 'bg-emerald-50 text-emerald-600',
  chart: 'bg-gold-50 text-gold-600',
  wallet: 'bg-purple-50 text-purple-600',
  time: 'bg-gold-50 text-gold-600',
  rank: 'bg-amber-50 text-amber-600',
  trophy: 'bg-amber-50 text-amber-600',
  bolt: 'bg-emerald-50 text-emerald-600',
...
function getMagnitudeInfo(val, prefix = '') {
  ...
  return {
    compact: `${prefix}${num.toLocaleString()}`,
    unit: 'Units',
    label: prefix === '$' ? `${num.toLocaleString()} Dollars` : `${num.toLocaleString()} Total`,
  };
}
...
export default function KPICard({
  ...
  const bgClass = bgMap[icon] || 'bg-gold-50 text-gold-600';
  ...
  return (
    ...
    >
      ...
      {/* Middle: Title */}
      <p className="text-xs sm:text-sm text-gray-500 font-medium mb-1 font-poppins">{title}</p>
      ...
      <div ref={containerRef} className="w-full overflow-hidden my-1">
        ...
        >
          ...
          <span className="inline-flex items-center font-extrabold text-gray-900 tracking-tight font-display">
          ...
          {suffix && (
            <span
              className="text-gray-400 font-semibold tracking-wider uppercase ml-1.5 select-none font-poppins"
      ...
      {/* Bottom: Auto-Calculated Magnitude & Subtitle Text (DESIGN.MD Matching) */}
