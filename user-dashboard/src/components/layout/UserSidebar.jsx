...
const navSections = [
  {
    ...
    items: [
      ...
      { path: '/plans', label: 'Investment Plans', icon: RiFundsLine, activeIcon: RiFundsFill },
      { path: '/investments', label: 'My Investments', icon: RiLineChartLine, activeIcon: RiLineChartFill },
  ...
  },
  {
    ...
    items: [
      { path: '/deposit', label: 'Deposit Funds', icon: RiArrowDownLine, activeIcon: RiArrowDownLine },
      { path: '/withdraw', label: 'Withdraw Funds', icon: RiArrowUpLine, activeIcon: RiArrowUpLine },
    ]
  },
  {
...
export default function UserSidebar({ isOpen, onToggle, isMobile }) {
  ...
  useEffect(() => {
    const handleAvatarSync = (e) => {
      setUserAvatar(e.detail !== undefined ? e.detail : (localStorage.getItem('horizon_user_avatar') || ''));
    };
      ...
      >
        ...
        <div className={`flex items-center border-b border-slate-100 flex-shrink-0 ${
          ...
          {(isOpen || isMobile) ? (
            ...
            /* Collapsed Luxury Gold Emblem with Official Logo Image */
            <div
        ...
        {/* ──────────────── BOTTOM FOOTER CARD WITH LIGHT GOLD AVATAR ──────────────── */}
          ...
          {(isOpen || isMobile) ? (
            <div className="p-2.5 bg-slate-50/90 rounded-2xl border border-slate-200/80 flex items-center justify-between gap-2 shadow-2xs">
              <div className="flex items-center gap-2.5 min-w-0">
                {/* Light Gold User Avatar Circle (Matching Super Admin) */}
                <div className="w-8 h-8 rounded-full overflow-hidden flex items-center justify-center flex-shrink-0 ring-2 ring-gold-200 bg-gradient-to-br from-gold-300 via-gold-400 to-amber-500 text-slate-950 font-bold text-xs shadow-2xs">
                  {userAvatar ? (
